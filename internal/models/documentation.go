...
func BuildDocumentationPrompt(doc ResourceDocumentation) string {
	...
	if len(doc.RequiredPermissions) > 0 {
		fmt.Fprintf(&b, "Permissions required to read this resource type: %s\n", strings.Join(doc.RequiredPermissions, ", "))
	}
	if doc.Lifecycle != "" {
		fmt.Fprintf(&b, "Lifecycle notes for this resource type: %s\n", doc.Lifecycle)
	}
	...
	b.WriteString("a metadata line stating the resource type, the concrete subtype (`@odata.type`, if present) and the resource ID, ")
	...
	b.WriteString("2. Settings — document EVERY setting/property present in the YAML in a table with the columns: ")
	...
	b.WriteString("4. Embedded payloads — fully expand and explain any embedded or encoded payloads")
	...
	b.WriteString("5. Security — call out security-sensitive settings (secrets, certificates, encryption, conditional-access conditions, etc.) and any deviations from recommended baselines, including the security impact.")
	if len(doc.KeySettings) > 0 {
		fmt.Fprintf(&b, " For this resource type, give particular attention to: %s.", strings.Join(doc.KeySettings, ", "))
	}
	b.WriteString("\n")
	b.WriteString("6. Lifecycle & operations — document operational guidance: deprecation or migration status, what happens when the resource is deleted or unassigned, ")
